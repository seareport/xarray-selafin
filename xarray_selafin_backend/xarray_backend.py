...
from .telemac_file import TelemacFile
...
def get_class(file, **kwargs):
    if HERMES_AVAIL:
        return TelemacFile(file, **kwargs)
    else:
        return Selafin(file, **kwargs)
    ...
    def write(self, filepath, **kwargs):
        ...
        if HERMES_AVAIL:
            if os.path.exists(filepath):
                os.remove(filepath)
            slf_writer = get_class(filepath, access="w")
            slf_writer.setMetaData(ds)
            slf_writer.set_values(ds)
            slf_writer.write()
        else:
            slf_writer = get_class("")
