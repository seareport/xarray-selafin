...
from xarray_selafin_backend.selafin import get_endian_from_char, \
...
class SerafinFile():
    ...
    def __read_bnd(self):
        ...
        self.__lihbor = np.zeros((self.__nptfr), dtype=int)
        self.__liubor = np.zeros((self.__nptfr), dtype=int)
        self.__livbor = np.zeros((self.__nptfr), dtype=int)
        ...
        self.__litbor = np.zeros((self.__nptfr), dtype=int)
        ...
        self.__nbor = np.zeros((self.__nptfr), dtype=int)
        self.__color = np.zeros((self.__nptfr), dtype=int)
    ...
    def get_mesh_coord(self, jdim):
        ...
        raise Exception("jdim must be within [1,{}]"\
                               .format(self.__ndim))
    ...
    def get_bnd_connectivity(self):
        ...
        """
        ikle_bnd = np.zeros((self.__nptfr, 1), dtype=int)
