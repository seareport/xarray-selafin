...
from .hermes.polygon import points_in_poly
from .hermes.hermes import HermesFile, ndp2elem, elem2str
from .hermes.linspace import \
        linspace_poly, curvilinear_abscissa
from .hermes.serafin_file import SerafinFile
try:
    import _hermes
    HERMES_AVAIL = True
except ImportError:
...
class TelemacFile(HermesFile if HERMES_AVAIL else SerafinFile):
    ...
    def ipob3(self):
        ...
        if self._ipob3 is None:
            if self.boundary_file is not None:
                ...
                self._ipob3 = np.zeros((self.npoin3), dtype=int)
    ...
    def meshx(self):
        """
        Returns the X coordinate of all mesh nodes.
        """
    ...
    def meshy(self):
        """
        Returns the Y coordinate of all mesh nodes.
        """
    ...
    def meshz(self):
        """
        Returns the Z coordinate of all mesh nodes.
        """
        if self._meshz is None and self.ndim == 3:
    ...
    def x_orig(self):
        """
        Returns the X coordinate of the mesh origin.
        """
    ...
    def y_orig(self):
        """
        Returns the Y coordinate of the mesh origin.
        """
    ...
    def get_liq_bnd_info(self):
        ...
        coords = np.zeros((2, self.npoin2), dtype=float)
        coords[0, :] = self.meshx
        coords[1, :] = self.meshy
    ...
    def get_data_on_points(self, varname, record, points):
        ...
        if len(np.shape(np.array(points))) != 2:
            raise Exception('Warning problem with the list of '
                                   'extraction points')
        ...
        if dim == 2:
            ...
            raise Exception('Warning problem with the dimension of '
                                   'extraction points')
        return res
    ...
    def _get_data_on_2d_points(self, varname, record, points):
        ...
        if self.get_mesh_dimension() != 2:
            raise Exception("Action possible only on 2d mesh")

        values = self.get_data_value(varname, record)
        if len(values) > self.npoin2:
            raise Exception('Warning the dimension of the result '
    ...
    def _get_data_on_3d_points(self, varname, record, points):
        ...
        if self.get_mesh_dimension() != 3:
            raise Exception("Action possible only on 3d mesh")
    ...
    def get_data_on_polyline(self, varname, record, polyline_points,
        ...
        if dim == 2:
            ...
            raise Exception('Warning the extraction on a polyline'
                                   ' is valid only in 2d')
    ...
    def get_data_on_horizontal_slice(
        ...
        @param varname (string) Name of variable for which to extract data
        @param record (int) Number of desired record
        @param zslices (numpy.array/list/float) Elevation of the slice or list
        of elevations
        ...
        if isinstance(zslices, (list, np.ndarray)):
            if isinstance(zslices, np.ndarray):
                if zslices.ndim > 1:
                    raise Exception('Warning the slice coordinate'
                                           'must be 1d')
            ...
            raise Exception(f'Unknown zslices type: {type(zslices)}')
        ...
        if z_name is None:
            raise Exception('Variable for elevation is missing')
    ...
    def get_data_on_vertical_segment(self, var_name, record, point):
        ...
        if len(point) != 2:
            raise Exception('Warning the extraction point '
                                   'must be 2d')
        nplan = self.nplan
        res = np.zeros(nplan)
        for plan in range(self.nplan):
    ...
    def get_spectrum_varname(self, point):
        ...
        if spectrum_var is None:
            raise Exception("Could not find point {} in your variables:\
    ...
    def add_mesh(self, x, y, ikle, z=None, nplan=0, orig=(0, 0)):
        ...
        self._nptfr = 0
        self._nptir = 0
        self._ipob3 = np.zeros((self._npoin3), dtype=int)
        self._knolg = np.zeros((self._npoin3), dtype=int)
    ...
    def add_bnd(self, ikle, lihbor=None, liubor=None, livbor=None,
        ...
        if lihbor is None:
            lihbor = np.ones((self._nelebd), dtype=int) * 2
        if liubor is None:
            liubor = np.ones((self._nelebd), dtype=int) * 2
        if livbor is None:
            livbor = np.ones((self._nelebd), dtype=int) * 2
        if hbor is None:
        ...
        if litbor is None:
            litbor = np.ones((self._nelebd), dtype=int) * 2
        if tbor is None:
        ...
        if color is None:
            color = np.array([i+1 for i in range(self._nelebd)], dtype=int)
    ...
    def add_data_value(self, varname, record, values):
        ...
        if isinstance(values, (int, float)):
            ...
            raise Exception("type {} is not handled for values"
                                   .format(type(values)))
    ...
    def write_mesh(self):
        ...
        for variable in ['title', 'varnames', 'varunits', 'nvar']:
            if getattr(self, "_"+variable) is None:
                raise Exception("Missing {} in class".format(variable))
    ...
    def getMesh(self):
        return self.nelem2, self.nelem3, self.npoin2, self.npoin3, \
            self.ndp2, self.ndp3, self.nplan, self.ikle2, self.ikle3, \
            self.ipob2, self.ipob3, self.meshx, self.meshy

    def get_values(self, itime):
        values = np.zeros((self.nvar, self.npoin3))
        for j in range(self._nvar):
            values[j] = self.get_data_value(self._varnames[j], itime)
        return values
    ...
    def get_iparam(self):
        ...
        iparam[6] = self._nplan
        iparam[7] = self._nptfr
        iparam[8] = self._nptir
    ...
    def setVars(self, ds):
        ...
        self.varunits = ds.varunits
        self.nbv1 = len(self.varnames)
        self.nvar = self.nbv1
        self.varindex = range(self.nvar)

    def setTimes(self, ds):
        self._times = ds.time.data
        self._ntimestep = len(self.times)
    ...
    def setGeometry(self, ds):
        ...
        """
        if 'plan' in ds.dims:
            self._nplan = len(ds.plan)
            if self._nplan == 1: self._nplan = 0
        else:
            self._nplan = 0
        if self.nplan > 1:
            # Adding additional metadata as attributes
            self._nelem2 = ds.attrs["nelem2"]
            self._nelem3 = ds.attrs["nelem3"]
            self._npoin2 = ds.attrs["npoin2"]
            self._npoin3 = ds.attrs["npoin3"]
            self._ipob2 = ds.attrs["ipob2"]
            self._ipob3 = ds.attrs["ipob3"]
            self._ndim = 3
            self._ndp2 = ds.attrs["ndp2"]
            self._ndp3 = ds.attrs["ndp3"]
            self._ikle2 = ds["ikle2"].data
            self._ikle3 = ds["ikle3"].data
            self.iparam = list(ds.attrs["iparam"])
        else:
            self._nelem2 = ds.attrs["nelem2"]
            self._nelem3 = self.nelem2
            self._npoin2 = ds.attrs["npoin2"]
            self._npoin3 = self.npoin2
            self._ipob2 = ds.attrs["ipob2"]
            self._ipob3 = self.ipob2
            self._ndp2 = ds.attrs["ndp2"]
            self._ndp3 = self.ndp2
            self._ndim = 2
            self._ikle2 = ds["ikle2"].data
            self._ikle3 = self.ikle2
            self.iparam = list(ds.attrs["iparam"])
        # calling properties
        self.nptfr
        self.nptir
        ...
        self.y_orig
        # Mesh coordinates
        self._meshx = ds.attrs['meshx']
        self._meshy = ds.attrs['meshy']

    def setMetaData(self, ds):
        self.setVars(ds)
        self.setTimes(ds)
        self.setGeometry(ds)
        self.add_header("Converted from Xarray", date=ds.attrs["date"])
